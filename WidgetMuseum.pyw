# -*- coding: utf-8 -*-
"""
Created on Fri Nov 21 00:25:46 2014
...
radio_button_group_variable.set("none")
vert_scale_variable = tk.IntVar()
hori_scale_variable = tk.IntVar()


def mdialog():
    tkMessageBox.showinfo("Info", "I am an info box")
    
def wdialog():
    tkMessageBox.showwarning("Warning", "Danger Will Robinson")

def edialog():
    tkMessageBox.showerror("Error", "Windows has experienced an error...but" \
                                    " I'll never tell. :D")
                                    
def askq():
    res = tkMessageBox.askyesno("Annoying","Would you like to continue?")
    if res == False:
        res = tkMessageBox.askquestion("Annoying","Are you sure?")
        if "yes" in res:
            res = tkMessageBox.askokcancel("Annoying", "This will close the app.")
            if res:
                while res:
                    res = tkMessageBox.askretrycancel("Annoying", "Just kidding.")
...
def radio_action(lbl):
    lbl.configure(text=radio_button_group_variable.get())
...
def initialize():
    ...
    entry_btn=tk.Button(entry_frame, text="Click", 
                        command=lambda:tkMessageBox.showinfo("Entry", entry.get()))
    entry.pack()
    entry_btn.pack()
