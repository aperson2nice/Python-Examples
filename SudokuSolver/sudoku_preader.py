...
def is_consistent(x,y, plist, charlst):
...
def sudoku_box_generator(x,y,plist):
    ...
    if y in [0,1,2,3]:
        ...
        new_y = 3
    print("\nX IS:", x, "Y IS:", y)
    return sudoku_box(new_x, new_y, plist)
...
def available_answers(plist, coordinates):
    ...
    for coordinate in coordinates:
        ...
        the_set = set(character_list) - have_set
        dict_of_possible_answers[(coordinate[0], coordinate[1]) ] = the_set
        print(dict_of_possible_answers[(coordinate[0], coordinate[1])])
...
def add_chars(lst): 
    ...
    list_of_chars = ['1','2','3','4','5','6','7','8','9','A','B','C','D','E','F','G']
    while len(lst) < 16:
...
f = open('SudokuPuzzle5.txt', 'r')
puzzle_string = f.read()
f.close()
puzzle_list = puzzle_string.splitlines() # get all the lines from the file
...
character_list = solution_list.keys()
...
#TESTING BOX SOLUTION FUNCTION
b = sudoku_box(1,2, puzzle_list)
print(b)
a = box_solution(puzzle_list,character_list)
print(a)
...
#TESTING CHECK EACH SPOT IN THE COORDINATE LIST
available_answers(puzzle_list, list_of_needs)
